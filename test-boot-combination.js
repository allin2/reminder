 ...
 *   · **本文件** —— 「`index.html` 里那 8 支脚本、按那个顺序、真的能起来吗」。
 *
...
function makeNode(id) {
  ...
  const node = {
    ...
    appendChild() {}, remove() {}, insertBefore() {}, scrollIntoView() {}
  };
...
function loadCombination(options) {
  ...
  const document = {
    ...
    createElement() { return makeNode("tmp"); },
    body: { appendChild() {} }
  };
  ...
  const sandbox = {
    console, Date, Math, JSON: JSONWrap, Object, Array, String, Number, Boolean, Set, Map,
...
async function run() {
  ...
  {
    ok("index.html 里解析出 8 支外部脚本", indexScripts.length === 8, JSON.stringify(indexScripts));
    ...
    // 加载期依赖：native-reminders 在求值时就把 AttentionLib.deadlineStageKey/Points
    // 抓进闭包常量；那三个标识符在 lib/reminder.js 里定义。
    const dep = prod.LOAD_TIME_DEPENDENCIES[0];
    ok("加载期依赖成立：lib/native-reminders.js 在 lib/reminder.js 之后",
      indexScripts.indexOf(dep.script) > indexScripts.indexOf(dep.requires[0]));
    ...
    const negUnknown = prod.inspectList(ROOT, ["lib/parse-cn.js", "scripts/verification/parse-parity.js", "app-core.js"]);
    ok("未知脚本 ⇒ 报 unknown-script",
      negUnknown.length === 1 && /^unknown-script:/.test(negUnknown[0]), JSON.stringify(negUnknown));
    const negMissing = prod.inspectList(ROOT, ["lib/parse-cn.js", "lib/does-not-exist.js", "app-core.js"]);
  ...
  section("A4. test-smoke.js / test-regressions.js 加载了哪些生产脚本（缺口必须是被声明的，不能是隐形的）");
  {
    ...
    // 免得有人看到上面「有缺口」就以为没人管。
    ok("缺口有明确补位：本 harness 加载了 lib/native-reminders.js（B 段）",
      prod.readIndexScripts(ROOT).indexOf("lib/native-reminders.js") >= 0 &&
      indexScripts.length === 8);
    // 反向对照：假如某套 harness 只声明加载了 7 支却不说明理由，覆盖判定必须报错 ——
  ...
  let boot = null;
  {
    ...
    ok("8 支脚本全部加载成功", boot.loaded.length === 8 && boot.loadErrors.length === 0,
