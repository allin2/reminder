...
(function (root, factory) {
   ...
   */
  /*
   * `essential`：缺了它提醒会直接失效或大幅延迟 —— 只有这类缺口才在首页出卡片；
   * 其余步骤是「提高可靠性 / 验证」，只在「我的 → 提醒设置」清单里出现。
   *
   * `homeDone`（可选）：首页卡片用它判断这一步是否已解决，缺省时沿用 `done`。
   * 「防冻结」需要它：`done` 在用户点进过设置页（visited）时就为真，用来推进向导；
   * 但点进去不代表开关真的开了。vivo 等机型上后台被冻结时，锁屏闹钟会一直挂起到
   * 亮屏（docs/reviews/android-vivo-freezer-rootcause-20260917.md）——
   * 所以首页只接受系统能回读的后台能力，或一次被确认的 60 秒测试。
   */
  function setupSteps(native, ctx) {
    ...
    // 原生状态里这三项能力只在 `diag` 下回读（canDrawOverlays / canUseFullScreenIntent /
    // ignoringBatteryOptimizations）；顶层字段形态保留兼容。只读顶层时，真机上已开启的
    // 能力永远判为未确认，向导会反复要求用户去开一个早已开着的权限。
    const diag = native.diag && typeof native.diag === "object" ? native.diag : {};
    const backgroundVerified = native.ignoringBatteryOptimizations === true ||
      diag.ignoringBatteryOptimizations === true;
    const overlayVerified = (native.overlay === "granted" || diag.canDrawOverlays === true) &&
      (native.fullScreenIntent === "granted" || diag.canUseFullScreenIntent === true);
    const steps = [];
    steps.push({
      id: "notify",
      essential: true,
    ...
    steps.push({
      ...
      homeDone: backgroundVerified || testVerified,
      title: "允许完全后台运行 (防冻结)",
