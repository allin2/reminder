...
(function(root, factory) {
  ...
  function createAppSetup(d) {
     ...
     * 本次绑定登记过的监听器（回滚用）。
     *
     * 绑定是**一个事务**：中途抛错时先把已挂上的监听器摘掉再上抛，重试才是一次干净的
     * 起点；只把 `bound` 挪到末尾只能救「第一行就抛」，救不了「绑到一半才抛」。
     */
    let registered = [];
  ...
  function renderSetupEntry() {
    ...
    // 原生状态还没读回（初始占位是 "unknown"）时不出卡片：否则冷启动会先闪一句
    // 「还差 5 步：允许发通知」，哪怕通知早已授权。状态回来后由 onStatusChange 重绘。
    if (nativeReminderStatus.notifications === "unknown") { writeIfChanged(host, ""); return; }
    const feedbackApi = FeedbackLib;
    const st = feedbackApi ? feedbackApi.setupSteps(nativeReminderStatus, setupStepsContext()) : null;
    // 首页只为**会让提醒直接失效或挂起**的缺口出卡片：通知、精确提醒、防冻结。
    // 悬浮窗与 60 秒测试是可选项，留在「我的 → 提醒设置」。
    // 防冻结按 `homeDone` 判断：点进过设置页不算解决，要系统能回读到后台能力，
    // 或 60 秒测试被确认；卡片始终可以用 × 关掉，不会成为关不掉的常驻提示。
    const missing = st && Array.isArray(st.steps)
      ? st.steps.filter(s => s.essential && !(typeof s.homeDone === "boolean" ? s.homeDone : s.done))
      : [];
    if (!missing.length) { writeIfChanged(host, ""); return; }
    const html = '<div class="setup-card">' +
      '<button class="soft-entry" id="setupEntry">' +
      "<span><strong>还差 " + missing.length + " 步，提醒才能准时响</strong><br>" +
      '<span class="setup-card-sub">下一步：' + escapeHtml(missing[0].title) + "</span></span>" +
      '<span style="color:var(--muted)">›</span></button>' +
      '<button class="setup-card-close" id="setupEntryDismiss" aria-label="不再在首页提示">' +
    ...
    const close = $("#setupEntryDismiss");
    if (close) close.addEventListener("click", () => dismissSetupEntry());
  }

  /** 用户在首页关掉设置卡片：不再在首页出现，设置入口仍在「我的 → 提醒设置」。 */
  function dismissSetupEntry() {
    state.settings.setupDismissed = true;
    save();
    renderSetupEntry();
    toast("好的，之后可在「我的 → 提醒设置」里继续");
  }
  ...
  function renderSetupSheetBody() {
    ...
    const permSteps = st.steps.filter(s => s.id !== "test");
    const missing = permSteps.filter(s => !s.done);
    const cur = missing[0] || null;
    // 清单：一眼看到全部步骤与各自状态，任何一步都能单独（重新）打开；
    // 当前建议的下一步高亮并展开说明，其余只显示标题，保持一屏可读。
    let html = '<p class="demo-note">' + (missing.length
      ? "还有 " + missing.length + " 项建议设置。每一项都可以跳过，跳过不影响记录事项。"
      : "建议的设置都处理过了。是否真正生效，以下面的 60 秒测试为准。") + "</p>";
    html += '<div class="setup-list">' + permSteps.map(s => {
      const isCur = cur && s.id === cur.id;
      return '<div class="setup-step' + (s.done ? " done" : "") + (isCur ? " current" : "") + '">' +
        '<div class="setup-step-head"><h4>' + escapeHtml(s.title) +
        (s.essential ? "" : '<span class="setup-tag">可选</span>') + "</h4>" +
        '<button type="button" class="chip' + (isCur ? " on" : "") + '" data-setup-step="' + escapeHtml(s.id) + '">' +
        escapeHtml(s.done ? "重新打开" : s.action) + "</button></div>" +
        (isCur ? "<p>" + escapeHtml(s.why) + "</p><p>跳过会怎样：" + escapeHtml(s.denyImpact) + "</p>" : "") +
        "</div>";
    }).join("") + "</div>";
    ...
    html += '<div class="setup-step' + (testDone ? " done" : "") + '"><h4>60 秒测试' +
      (testDone ? " · 本次已出结果" : "") + "</h4>" +
      "<p>点开始后锁屏等一分钟，看看这台手机上提醒会不会响、会不会亮屏。测试不会生成事项。</p>" +
      '<div class="setup-test-actions">' +
      '<button type="button" class="chip" id="setupTestStart">' +
      (testDone ? "再测一次" : "开始 60 秒测试") + "</button>" +
      '<button type="button" class="chip" id="setupTestStop">停止铃声 / 取消测试</button>' +
      "</div></div>" +
    ...
    if (btn) {
      btn.textContent = cur ? cur.action : (testDone ? "再测一次" : "开始测试");
      btn.onclick = () => runSetupStep(cur ? cur.id : "test");
    }
    $$("#setupBody [data-setup-step]").forEach(el => {
      el.addEventListener("click", () => runSetupStep(el.dataset.setupStep));
    });
    // 用户反馈
  ...
  function updateSetupEntry() {
    ...
    const feedbackApi = FeedbackLib;
    const st = feedbackApi ? feedbackApi.setupSteps(nativeReminderStatus, setupStepsContext()) : { steps: [], next: null };
    const missing = st.steps.filter(s => !s.done && s.id !== "test");
    sub.textContent = missing.length
      ? "还有 " + missing.length + " 项建议设置 · 60 秒测试"
      : "已设置好 · 可随时做 60 秒测试";
  }

  async function runSetupStep(stepId) {
    if (stepId === "notify") {
      // 已授权时系统不会再弹授权框 ——「重新打开」直接去系统通知设置
      if (nativeReminderStatus.notifications === "granted") {
    ...
    /**
     * 绑定静态 `#btnSetup`。**只在注册动作成功后**才置位 `bound`；
     * 中途抛错时回滚本次已登记的监听器并原样上抛，于是重试是干净的起点。
     */
    function bind() {
      if (bound) return;
      registered = [];
      try {
        const row = $("#btnSetup");
        if (row) listen(row, "click", openSetupSheet);
        bound = true;
      } catch (error) {
        rollbackListeners();
        throw error;
