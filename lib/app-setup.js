...
(function(root, factory) {
  ...
  function renderSetupEntry() {
    ...
    // 首页只为**会让提醒直接失效**的缺口（通知 / 精确提醒）出卡片。后台保活、悬浮窗
    // 无法可靠回读、60 秒测试是可选验证 —— 它们留在「我的 → 提醒设置」，
    // 否则这张卡片几乎永远消不掉，首页一直挂着「还没准备好」。
    const missing = st && Array.isArray(st.steps) ? st.steps.filter(s => !s.done && s.essential) : [];
