...
(function(root, factory) {
  ...
  function createAppNativeCoordinator(input) {
     ...
     * 回到前台后的原生状态补读。
     *
     * 用户从系统设置页（后台耗电、锁屏显示、精确提醒……）返回时，onResume 会立刻读一次；
     * 但部分厂商把改动写进系统是异步的 —— vivo 开关矩阵实测，切到「允许后台高耗电」后
     * 回到应用，第一次读到的 ignoringBatteryOptimizations 仍可能是旧值，要到下一次回前台
     * 才更新（docs/reviews/vivo-switch-matrix-2026-09-26.md §4.2）。于是只要还有能力没被
     * 验证，就稍后再补读两次，状态真有变化才回写；再次回前台会取消上一轮尚未执行的补读。
     */
    const RESUME_RECHECK_DELAYS_MS = [1500, 5000];
    let resumeRecheckTimers = [];

    function nativeStatusFullyVerified(s) {
      if (!s) return false;
      const d = s.diag && typeof s.diag === "object" ? s.diag : {};
      return s.notifications === "granted" && s.exactAlarm === "granted" &&
        (s.ignoringBatteryOptimizations === true || d.ignoringBatteryOptimizations === true) &&
        (s.overlay === "granted" || d.canDrawOverlays === true) &&
        (s.fullScreenIntent === "granted" || d.canUseFullScreenIntent === true);
    }

    function scheduleResumeStatusRechecks(nr) {
      resumeRecheckTimers.forEach(t => timerClear(t));
      resumeRecheckTimers = [];
      if (!nr || !nr.getPermissionState || nativeStatusFullyVerified(nativeReminderStatus)) return;
      RESUME_RECHECK_DELAYS_MS.forEach(ms => {
        resumeRecheckTimers.push(timerSet(async () => {
          try {
            const next = await nr.getPermissionState();
            if (!next) return;
            const merged = Object.assign({}, nativeReminderStatus, next);
            if (JSON.stringify(merged) === JSON.stringify(nativeReminderStatus)) return;
            setNativeReminderStatus(next, "resume-recheck");
            deps.renderMe();
          } catch (error) {}
        }, ms));
      });
          ...
          onResume: async () => {
            try {
              if (nr.getPermissionState) {
                ...
                deps.renderMe();
                scheduleResumeStatusRechecks(nr);
              }
            } catch (error) {}
            try {
