...
(function (root, factory) {
  ...
  /*
   * 已送达通知的归属索引（本机 localStorage，不进备份）。
   *
   * Android 上 `getDeliveredNotifications()` 只回 id / 标题 / 正文，不带排程时的 `extra`
   * （2026-09-25 vivo V2238A / Android 16 实测），无法从已送达通知本身认出它属于哪条事项。
   * 所以在通知还在 pending（带完整 extra）或由本轮排下时记下 id → {itemId, itemRev}，
   * 送达后再据此判断它是否已经陈旧：事项已删除、已进入终态，或送达后事项版本已推进
   * （ACK / 完成 / 稍后 / 改时间都会 bumpRev；单纯到期不会）。
   * 缺了这一步，删除或在应用内处理过的事项，其已送达通知会一直留在通知栏，
   * 用户点它的按钮时操作的是一个已不存在或已变化的事项。
   */
  const DELIVERED_INDEX_KEY = "attention-inbox-delivered-notif-index";
  const DELIVERED_INDEX_MAX = 200;

  function deliveredIndexStore() {
    try {
  ...
  function readDeliveredIndex() {
    const ls = deliveredIndexStore();
    if (!ls) return {};
    try {
      const raw = JSON.parse(ls.getItem(DELIVERED_INDEX_KEY) || "{}");
  ...
  function writeDeliveredIndex(index) {
    const ls = deliveredIndexStore();
    if (!ls) return;
    const ids = Object.keys(index);
    if (ids.length > DELIVERED_INDEX_MAX) {
      ids.sort((a, b) => (index[a].seenAt || 0) - (index[b].seenAt || 0))
        .slice(0, ids.length - DELIVERED_INDEX_MAX)
        .forEach(id => { delete index[id]; });
    }
    try { ls.setItem(DELIVERED_INDEX_KEY, JSON.stringify(index)); } catch (error) {}
  }

  function indexNotification(index, n, now) {
    const extra = n && n.extra;
    if (!extra || extra.managedKind !== MANAGED_KIND || !extra.itemId) return;
    index[String(n.id)] = {
      itemId: String(extra.itemId),
      itemRev: extra.itemRev == null ? 0 : Number(extra.itemRev),
      seenAt: now
    };
  }

  /** 已送达通知是否陈旧：事项不在了、已终态，或送达后事项版本已推进。 */
  function deliveredNotificationStale(entry, itemsById) {
    const item = itemsById.get(entry.itemId);
    if (!item) return true;
    if (item.status === "completed" || item.status === "archived") return true;
    const rev = item.rev == null ? 0 : Number(item.rev);
    return rev !== Number(entry.itemRev || 0);
  }

  /**
   * 撤掉陈旧的已送达事项通知，并维护归属索引。
   * pending 读取失败时不清理索引（无法区分「已送达」与「还在排」），但仍按已知条目撤陈旧通知。
   */
  async function removeStaleDelivered(local, items, pending, scheduled, pendingReadOk, now) {
    const result = { removed: [], error: null };
    if (!local || typeof local.getDeliveredNotifications !== "function" ||
      typeof local.removeDeliveredNotifications !== "function") return result;
    const index = readDeliveredIndex();
    pending.forEach(n => indexNotification(index, n, now));
    scheduled.forEach(n => indexNotification(index, n, now));
    let delivered = [];
    try {
      const r = await local.getDeliveredNotifications();
      delivered = Array.isArray(r && r.notifications) ? r.notifications : [];
    } catch (error) {
      result.error = "notifications:delivered:" + String(error.message || error);
      writeDeliveredIndex(index);
      return result;
    }
    const itemsById = new Map((items || []).filter(it => it && it.id).map(it => [String(it.id), it]));
    const pendingIds = new Set(pending.map(n => String(n.id)));
    const deliveredIds = new Set(delivered.map(n => String(n.id)));
    const stale = delivered.filter(n => {
      const entry = index[String(n.id)];
      // 送达的 id 仍在 pending：id 已被本轮新排程复用，索引描述的是新排程，不是这条已送达通知
      return entry && !pendingIds.has(String(n.id)) && deliveredNotificationStale(entry, itemsById);
    });
    if (stale.length) {
      try {
        await local.removeDeliveredNotifications({ notifications: stale.map(n => ({ id: n.id })) });
        result.removed = stale.map(n => n.id);
        stale.forEach(n => { delete index[String(n.id)]; });
      } catch (error) {
        result.error = "notifications:remove-delivered:" + String(error.message || error);
      }
    }
    // 只在 pending 可靠读到时修剪：既不在 pending、也不在通知栏的条目已无意义
    if (pendingReadOk) {
      Object.keys(index).forEach(id => {
        if (!pendingIds.has(id) && !deliveredIds.has(id) &&
          !scheduled.some(n => String(n.id) === id)) delete index[id];
      });
    }
    writeDeliveredIndex(index);
    return result;
  ...
  async function reconcile(items, settings, now, review, options) {
    ...
    // 撤掉陈旧的已送达事项通知（事项已删除 / 终态 / 送达后已变化）。
    // 与排程无关：总开关关闭或无通知权限时，旧通知同样不该留着让用户点。
    // 撤不掉只影响通知栏整洁，不影响后续提醒能否送达，所以单独回报，不进 errors / reliability。
    let staleDelivered = { removed: [], error: null };
    if (local) {
      staleDelivered = await removeStaleDelivered(local, items, pending, scheduled, pendingReadOk,
        now == null ? Date.now() : Number(now));
    }
    ...
    const status = Object.assign({}, permissions, {
      ...
      errors,
      // 本轮撤掉的陈旧已送达通知 id，以及撤除失败的诊断（不计入 errors）
      removedDeliveredIds: staleDelivered.removed,
      deliveredCleanupError: staleDelivered.error
    });
