...
(function (root, factory) {
  ...
  function buildReviewDesired(count, reviewSettings, now, settings) {
    ...
    // D18：落在勿扰时段内的时刻顺延到勿扰结束（与普通事项同一规则）
    function shift(ts) {
      if (!ts) return 0;
    ...
    if (snoozedUntil > now) times.push(shift(snoozedUntil));
    // 勿扰只作用于锚点（窗口起点）；补提醒按固定间隔从「已顺延的锚点」往后推，
    // 否则三条会被各自顺延到同一个时刻而被去重压成一条。
    const anchor = shift(start.getTime());
    times.push(anchor);
    for (let i = 1; i <= maxFollowups; i++) {
      times.push(anchor + i * followupMs);
    }

    const slots = Array.from(new Set(times.filter(t => t > now))).sort((a, b) => a - b);
    return slots.map((at, i) => reviewNotificationFor(count, i, at, usedIds));
  }
  ...
  async function reconcileAlarms(desired, items, settings, now) {
    ...
    if (!bridge || (!bridge.scheduleAlarm && !bridge.scheduleAt)) {
      ...
      return { available: false, cancelled: 0, scheduled: 0, ids: prev };
    }
    ...
    return { available: true, cancelled, scheduled, ids: wanted };
  }
