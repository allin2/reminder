...
function createAppViews(deps){deps=deps||{};var need=['query','queryAll','getState','openSheet','openCapture','openDemoPreview','isAttentionDue','needsReviewItems','renderReviewEntry','inReviewHighlight','renderAiSub','ensureReviewSettings','updateSetupEntry','isTerminal','updateAppBadge','fmtTime','fmtDate','relDue','repeatLabel','dayLabel','escapeHtml','escapeAttr','safeExternalHref','sameDay','startOfDay','pad2','getNativeReminderStatus','getNativeReminders','getSwReg','isNativeAndroidRuntime','evidenceStatusFor','deliveryEvidenceReadable','itemScheduleEvidence'];var missing=need.filter(function(k){return typeof deps[k]!=="function";});if(missing.length)throw new Error("AppViews missing dependencies: "+missing.join(','));var $=deps.query,$$=deps.queryAll,openSheet=deps.openSheet,openCapture=deps.openCapture,openDemoPreview=deps.openDemoPreview;var state=new Proxy({}, {get:function(_,k){return deps.getState()[k];},set:function(_,k,v){deps.getState()[k]=v;return true;}});var isAttentionDue=deps.isAttentionDue,needsReviewItems=deps.needsReviewItems,renderReviewEntry=deps.renderReviewEntry,inReviewHighlight=deps.inReviewHighlight,renderAiSub=deps.renderAiSub,ensureReviewSettings=deps.ensureReviewSettings,updateSetupEntry=deps.updateSetupEntry,isTerminal=deps.isTerminal,updateAppBadge=deps.updateAppBadge,fmtTime=deps.fmtTime,fmtDate=deps.fmtDate,relDue=deps.relDue,repeatLabel=deps.repeatLabel,dayLabel=deps.dayLabel,escapeHtml=deps.escapeHtml,escapeAttr=deps.escapeAttr,safeExternalHref=deps.safeExternalHref,sameDay=deps.sameDay,startOfDay=deps.startOfDay,pad2=deps.pad2,isNativeAndroidRuntime=deps.isNativeAndroidRuntime,evidenceStatusFor=deps.evidenceStatusFor,deliveryEvidenceReadable=deps.deliveryEvidenceReadable,itemScheduleEvidence=deps.itemScheduleEvidence;var homeViewSignature=null,homeRenderStats={builds:0,skipped:0,dueCards:0,activeCards:0};
  ...
  // 方案 A：到点卡片的三个动作带小图标，与文字一起区分语义（图标是装饰，不承载信息）
  const ACT_ICONS = {
    snooze: '<circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/>',
    ack: '<path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12z"/><circle cx="12" cy="12" r="3"/>',
  ...
  function actionButton(act, id, cls, label, sub, withIcon) {
    return '<button class="' + cls + '" data-act="' + act + '" data-id="' + escapeAttr(id) + '">' +
      '<span class="act-label">' + (withIcon && ACT_ICONS[act] ? actIcon(act) : "") + escapeHtml(label) + "</span>" +
  ...
  function renderItemCard(it, mode) {
    ...
    if (mode === "due") {
      actions = '<div class="card-actions">' +
        actionButton("snooze", it.id, "ghost", "稍后", "改到具体时间", true) +
        actionButton("ack", it.id, "primary", "我知道了", "停止本轮 · 仍未完成", true) +
        // 「完成」是唯一归档出口，用描边而不是实心：看得清，但不会被顺手误点成默认动作
        actionButton("done", it.id, "line", "完成", "结束并归档", true) +
        "</div>";
    } else if (mode === "active") {
      actions = '<div class="card-actions">' +
  ...
  /** 方案 A：未来列表的紧凑行 —— 左列「哪天 + 几点」，右侧标题与少量标签；点整行看详情 */
  function futureWhen(ts) {
    if (!ts) return '<div class="fut-when">未定<b>—</b></div>';
    const d = new Date(ts);
    const today = startOfDay(Date.now()).getTime();
    const diff = Math.round((startOfDay(ts).getTime() - today) / 86400000);
    const day = diff === 0 ? "今天" : diff === 1 ? "明天" : diff > 1 && diff < 7
      ? "周" + "日一二三四五六"[d.getDay()]
      : (d.getMonth() + 1) + "/" + d.getDate();
    return '<div class="fut-when">' + day + "<b>" + pad2(d.getHours()) + ":" + pad2(d.getMinutes()) + "</b></div>";
  }
  function renderFutureRow(it) {
    const pills = [];
    if (it.priority === "important") pills.push('<span class="pill warn">☆ 重要</span>');
    if (it.priority === "critical") pills.push('<span class="pill crit">🚨 关键</span>');
    const proj = projectById(it.projectId);
    ...
    (it.tags || []).forEach(t => pills.push('<span class="pill tag">#' + escapeHtml(t) + "</span>"));
    return '<article class="card fut-row ' +
      (it.priority === "critical" ? "urgent" : it.priority === "important" ? "important" : "") +
      '" data-id="' + escapeAttr(it.id) + '">' +
      futureWhen(it.triggerAt) +
      '<div class="fut-body"><div class="card-title">' + escapeHtml(it.title) + "</div>" +
      (pills.length ? '<div class="card-meta">' + pills.join("") + "</div>" : "") + "</div>" +
      '<button class="fut-edit" data-act="edit" data-id="' + escapeAttr(it.id) + '">修改</button>' +
      "</article>";
  }
  /** 按「今天 / 明天 / 本周 / 下周 / 更晚」分组（周一为一周开始） */
  function futureBucket(ts) {
    if (!ts) return "未设定时间";
    const today = startOfDay(Date.now());
    const diff = Math.round((startOfDay(ts).getTime() - today.getTime()) / 86400000);
    if (diff <= 0) return "今天";
    if (diff === 1) return "明天";
    const toWeekEnd = 6 - ((today.getDay() + 6) % 7);
    if (diff <= toWeekEnd) return "本周";
    if (diff <= toWeekEnd + 7) return "下周";
    return "更晚";
  }
  function setBadge(n) {
    const el = $("#navBadge");
  ...
  function renderHome() {
      ...
      $("#homeEmpty").innerHTML = quiet
        ? '<div class="empty"><div class="empty-mark">✓</div><h3>' +
          ...
          (reviewPending
            ...
            : "例如「明天下午3点提醒我取快递」") +
          "</p>" +
          // 首次使用（从没记过事项）时，把最核心的用法直接摆在空首页上，不用先去翻说明
          (!reviewPending && !state.items.length
            ? '<ol class="empty-steps">' +
              "<li>点「记一件事」，用一句话写下来</li>" +
              "<li>到点提醒你，没到点的事不打扰你</li>" +
              "<li>做完点「完成」，只是看到点「我知道了」</li>" +
              "</ol>"
            : "") +
          (doneToday.length
      ...
      if (homeStart) {
        ...
        homeStart.innerHTML = quiet
          ? '<div class="empty-start">' +
            ...
            (isBeginner ? '<button class="soft-entry" id="emptyGuide"><span>使用说明</span><span style="color:var(--muted)">›</span></button>' : "") +
            (isBeginner && !state.items.length ? '<button class="soft-entry" id="emptyDemo"><span>看看演示（不会写入数据）</span><span style="color:var(--muted)">›</span></button>' : "") +
            "</div>"
          : "";
    ...
    // 方案 A：首页标题是一句状态，日期作为眉题（CSS 在 .top.is-home 下把副标题放到标题上方）
    const today = new Date(now);
    const dateLine = (today.getMonth() + 1) + "月" + today.getDate() + "日 周" + "日一二三四五六"[today.getDay()];
    $("#pageTitle").innerHTML = dueList.length
      ? "有 <em>" + dueList.length + " 件事</em>需要你看一眼"
      : active.length ? "暂时没有新的提醒" : "现在很安静";
    $("#pageSub").textContent = dueList.length || active.length
      ? dateLine
      : dateLine + " · 可以放心忘记";
  }
  ...
  function renderFuture() {
    ...
    const groups = [];
    list.forEach(it => {
      const label = futureBucket(it.triggerAt);
    ...
    $("#futureList").innerHTML = filterNote + (list.length
      ? '<div class="sec">' +
        groups.map(g =>
          '<div class="fut-group"><span>' + escapeHtml(g.label) + "</span><span>" + g.items.length + "</span></div>" +
          g.items.map(renderFutureRow).join("")
        ).join("") +
      ...
      : '<div class="empty"><div class="empty-mark">⏱</div><h3>没有托管中的未来</h3>' +
        "<p>在底部输入框写一句话，把未来事项交出去，现在就可以少记挂。</p></div>");

    $("#pageTitle").textContent = "未来";
    $("#pageSub").textContent = "托管中 " + list.length + " 件 · 到点前不会出现在首页";
  }
  ...
  function renderStats() {
    ...
    const dndSub = $("#dndSub");
    if (dndSub) dndSub.textContent = qs + "–" + qe + " 普通事项延后 · 点此改时段";
  ...
  function renderMe() {
    ...
    if (sum) sum.textContent = state.settings.dailySummary ? "已开启 · 新增较多时给一句小结" : "默认关闭 · 新增较多时给一句小结";
    renderAiSub();
    renderStats();
    renderPwaStatus();
    $("#pageTitle").textContent = "我的";
    $("#pageSub").textContent = "设置、帮助与数据";
  }
  function renderPwaStatus() {
    ...
    // 通知 / 精确闹钟 / 后台保活的系统入口统一收进「提醒设置」清单与「高级诊断」，
    // 「我的」页不再平铺一排重复的系统设置跳转。
  ...
  function render() {
    ...
    const appEl = $("#app"), topEl = $(".top");
    if (appEl && appEl.classList) appEl.classList.toggle("on-me", state.ui.tab === "me");
    if (topEl && topEl.classList) topEl.classList.toggle("is-home", state.ui.tab === "home" || !state.ui.tab);
