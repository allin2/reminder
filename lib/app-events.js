...
(function(root, factory) {
  ...
  function createAppEvents(input) {
    ...
    /* U1（P3-I-R 独立复验 20260924）：稍后面板的选时状态由**本实例**持有。
     * 旧实现里这两个绑定是 bind() 的局部变量，入口 openSnoozeSheet 给它们赋值
     * 会直接 ReferenceError（严格模式），用户点「稍后提醒」面板打不开。 */
    let snoozePick = null;
     ...
     * 绑定过程登记过的监听器（回滚用）。
     *
     * 为什么必须有它：`bind()` 中间任何一步抛错（DOM 缺失、注入回调抛异常）时，
     * 已经挂上去的监听器必须被摘掉。否则「重试」会在半套监听器上再挂一套 ——
     * 一次点击触发两次动作，而调用方看不出任何异常（`bound` 已经是 `true`，
     * 却没有任何监听器活着）。
     *
     * 它同时是「重试不翻倍」的唯一保证：**只把 `bound = true` 挪到末尾是不够的**，
     * 那能救「早段抛错」，救不了「后段抛错」—— 后段抛错时前面那些监听器已经在 DOM 上，
     * 重试会把它们再挂一遍。
     */
    let registered = [];
     ...
     * 绑定的**事务体**：全部注册动作都在这里。
     *
     * 返回 `false` = 环境里没有 DOM，什么都没做（**不算成功**，也就不置 `bound`，
     * 留给下一次真正的机会）。抛出的异常由 `bind()` 统一回滚后原样上抛，
     * 让 `startApp()` 的失败提示与重试逻辑照常生效。
     */
    function bindListeners() {
      ...
      const fab = $("#fab");
      if (fab) listen(fab, "click", function() { deps.openCapture(); });

      const backdrop = $("#backdrop");
      if (backdrop) listen(backdrop, "click", deps.closeAllSheets);

      $$("[data-close]").forEach(function(b) {
        listen(b, "click", function() { deps.closeSheet(b.dataset.close); });
      });
      ...
      if (btnSaveItem) listen(btnSaveItem, "click", deps.saveItemFromForm);

      const capText = $("#capText");
      if (capText) {
        listen(capText, "keydown", function(e) {
      ...
      if (lowConfCustom) {
        listen(lowConfCustom, "change", function() {
      ...
      if (btnLowConfOk) {
        listen(btnLowConfOk, "click", function() {
          const state = deps.getState();
      ...
      listen(doc, "click", async function(e) {
      ...
      const snoozeCustom = $("#snoozeCustom");
      if (snoozeCustom) {
        listen(snoozeCustom, "change", function() {
      ...
      if (btnApplySnooze) {
        listen(btnApplySnooze, "click", function() {
          const state = deps.getState();
      ...
      if (swNotify) {
        listen(swNotify, "click", async function() {
          const state = deps.getState();
      ...
      if (btnExactAlarm) {
        listen(btnExactAlarm, "click", async function() {
      ...
      if (btnNotifySettings) {
        listen(btnNotifySettings, "click", async function() {
          if (!deps.isNativeAndroidRuntime()) return;
      ...
      if (btnBatterySettings) {
        listen(btnBatterySettings, "click", async function() {
          if (!deps.isNativeAndroidRuntime()) return;
      ...
      if (btnUserGuide) {
        listen(btnUserGuide, "click", function() { deps.openSheet("sheetGuide"); });
      }

      const btnSwitchToNormal = $("#btnSwitchToNormalFromGuide");
      if (btnSwitchToNormal) {
        listen(btnSwitchToNormal, "click", function() {
      ...
      if (swSummary) {
        listen(swSummary, "click", function() {
          const state = deps.getState();
      ...
      if (swPrivacy) {
        listen(swPrivacy, "click", function() {
          const state = deps.getState();
      ...
      if (btnQuiet) {
        listen(btnQuiet, "click", function() {
          const state = deps.getState();
      ...
      if (btnSaveQuiet) {
        listen(btnSaveQuiet, "click", function() {
          const state = deps.getState();
          const quietStart = $("#quietStart");
          const quietEnd = $("#quietEnd");
      ...
      const btnAi = $("#btnAi");
      if (btnAi) listen(btnAi, "click", function() { const a = getAi(); if (a) a.openAiSheet(); });

      const swAi = $("#swAi");
      if (swAi) {
        listen(swAi, "click", function() {
      ...
      if (btnSaveAi) listen(btnSaveAi, "click", function() {
      ...
      if (btnAiTest) {
        listen(btnAiTest, "click", async function() {
          const a = getAi();
      ...
      if (btnAiParse) listen(btnAiParse, "click", function() {
      ...
      if (btnAiPolish) listen(btnAiPolish, "click", function() {
      ...
      if (btnExport) listen(btnExport, "click", function() {
      ...
      if (btnImport && importFile) listen(btnImport, "click", function() { importFile.click(); });
      if (importFile) {
        listen(importFile, "change", function(e) {
      ...
      if (btnClear) {
        listen(btnClear, "click", async function() {
     ...
     * 绑定整个界面交互。**失败必须可恢复**：
     *
     *   · 只在**全部**注册动作成功后**才**宣布 `bound = true`（否则「绑了一半」
     *     会被当成「已经绑好了」）；
     *   · 中途抛错时把本次已登记的监听器**全部摘掉**再上抛，于是重试是一次干净的
     *     起点，而不是在半套监听器上再叠一套；
     *   · 被委托的模块（诊断 / 内容 / 表单 / 整理 / 提示条）各自也保证「失败回滚、
     *     可重试、成功后才置位」—— 它们已成功过一次时，重试会因自身的幂等标志短路，
     *     所以跨模块重试同样不会翻倍。
     */
    function bind() {
      if (bound) return;
     ...
     * U1 修复：面板打开与选时状态由本实例持有；事务待办闸门由入口以具名依赖注入。 */
    function openSnoozeSheet(id) {
      if (deps.isItemActionPending(id)) { deps.rejectPendingItemCommand(); return false; }
      const state = deps.getState();
      state.ui.snoozeId = id;
      snoozePick = null;
      snoozeBasis = "elapsed";
      const custom = $("#snoozeCustom");
      if (custom) custom.value = "";
      $$("#snoozeChips .chip").forEach(function(x) { x.classList.remove("on"); });
      deps.closeSheet("sheetDetail");
      deps.openSheet("sheetSnooze");
      return true;
    }
    ...
    return {
      bind: bind,
      isBound: isBound,
      openSnoozeSheet: openSnoozeSheet,
