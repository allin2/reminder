...
/**
 * 绑定：app-core 必须**显式**从 `AppUi` 取，而且这些名字都在用（不是取了个空壳）。
 *
 * ⚠️ F03 之后绑定形态变了：不再是 IIFE 顶部的 `const $ = appUi.$;`，而是
 * 「`let` 声明 + 装配函数里赋值」—— 因为「重试」必须能**重建**绑定。旧形态下
 * 闸门读的是实时 `Lib`、业务跑的是求值时抓到的旧函数，两者可以不是同一个
 * （这就是 F03：补载解析器后「重试」显示成功，输入/保存仍报错）。
 *
 * 所以判据也跟着换成两件事：① 每个名字确实由 `bindRuntime()` 绑定；
 * ② 旧的 const 捕获形态**一个都不剩**（否则「同源」只是口头承诺）。
 */
const bindStart = coreCode.indexOf("function bindRuntime()");
const bindEnd = coreCode.indexOf("function startApp()");
const BIND_BLOCK = (bindStart >= 0 && bindEnd > bindStart)
  ? coreCode.slice(bindStart, bindEnd) : "";
ok("装配函数 bindRuntime() 存在且非空（下面所有绑定判据的前提）", BIND_BLOCK.length > 0);

const DOM_BINDINGS = ["$", "$$", "openSheet", "closeSheet", "closeAllSheets",
  "confirmDialog", "toast", "hideToast", "safeExternalHref"];
DOM_BINDINGS.forEach(function (name) {
  // 注意 `$` / `$$`：它们不是单词字符，`\b` 在它们身上恒不成立（前一个字符也是
  // 空格/换行），用 `\b` 会得到一个永远为假的断言。这里按「行首或空白之后」判。
  ok("bindRuntime() 绑定「" + name + "」",
    new RegExp("(^|[\\s;])" + name.replace(/\$/g, "\\$") + "\\s*=[^=]", "m").test(BIND_BLOCK));
});
{
  const stale = DOM_BINDINGS.filter(function (name) {
    return new RegExp("const\\s+" + name.replace(/\$/g, "\\$") +
      "\\s*=\\s*(appUi|AppUi|Lib|DatePrimitives|UiFormat)\\.").test(coreCode);
  });
  ok("旧的 const 捕获形态一个都不剩（求值时抓一次就会被钉死）",
 ...
 * **活绑定**：抑制反馈必须按**取值函数**注入。
 *
 * 若按值传（`{ isFeedbackSuppressed: suppressUserFeedback }`），装配发生的那一
 * 刻它恒为 `false`，于是「重放用户操作时不重复弹提示」（M1）会整段失效 —— 而且**不报错**。
 * 这条断言把那个写法钉死。
 */
ok("抑制反馈按取值函数注入（按值传会让 M1 静默失效）",
  /factory\(\{\s*isFeedbackSuppressed:\s*\(\)\s*=>\s*(?:suppressUserFeedback|\(?appTransaction\b[\s\S]*?\)?)\s*\}\)/.test(coreCode));
ok("没有把抑制反馈按值传进去",
...
const UI_USAGE_SHAPES = [
  ["$(\"#", "$ 查询（弹层/toast 的取节点）"],
  /* U1（2026-09-24）：`$$("#snoozeChips .chip")` 这一处字面量调用随稍后面板
   * 迁入 lib/app-events.js 后，app-core 里 `$$` 的真实引用只剩注入模块 deps
   * 的 `queryAll: (sel, scope) => $$(sel, scope)` 形状 —— 按「仍被真实引用」
   * 的本意改为匹配该形状；「绑了却完全没人引用」仍会被本表点名。 */
  ["$$(sel", "$$ 批量查询（注入模块 deps 的取节点函数）"],
  ["openSheet(", "开弹层"],
  ["closeSheet(", "关弹层"],
...
const BINDINGS = [
  ["parseChineseTime", "解析器"],
  ["cnInt", "中文数字"],
  ["hasSpecificTimeWord", "明确时间词"],
  ["nextRepeatTrigger", "周期推进"],
  ["repeatLabel", "周期文案"],
  ["nextRepeatPreview", "周期预演"],
...
BINDINGS.forEach(function (b) {
  ok("bindRuntime() 绑定「" + b[0] + "」（" + b[1] + "）",
    new RegExp("\\b" + b[0] + "\\s*=[^=]").test(BIND_BLOCK));
});
{
  const stale = BINDINGS.filter(function (b) {
    return new RegExp("const\\s+" + b[0] + "\\s*=\\s*(Lib|DatePrimitives|UiFormat)\\.").test(coreCode);
  });
  ok("旧的 const 捕获形态一个都不剩（解析器 / 周期 / 日历原语）",
    stale.length === 0, JSON.stringify(stale.map(function (b) { return b[0]; })));
}
...

{
  ...
  /* ⚠️ 第三轮改制：**只看 undeclared 不足以说明闭合**。
   * 扫描器认不出归属的引用（同名局部变量既被赋成命名空间、又被赋成别的东西）
   * 从前被丢进「只报告、不判错」的桶里 —— 独立复验 F02 的反例正是从那里穿过的：
   * 把真实的 `f.setupSteps` 换成未声明成员，`undeclared` 仍返回 `[]`。
   * 所以现在判四件事，任何一个非空都算**没闭合**。 */
  ok("产品源码没有歧义别名（一名两义已被消除）",
    cov.ambiguousAliases.length === 0, JSON.stringify(cov.ambiguousAliases));
  ok("产品源码没有歧义引用（不存在「证不了归属就跳过」的残留）",
    cov.ambiguousRefs.length === 0, JSON.stringify(cov.ambiguousRefs.slice(0, 8)));
  ok("唯一判定出口 problems 为空（undeclared + 两类歧义都进来）",
    cov.problems.length === 0, JSON.stringify(cov.problems));
  ok("别名归一生效：`feedbackApi` ⇒ `FeedbackLib`（别名调用确实落在声明路径上）",
    cov.aliases.feedbackApi === "FeedbackLib" &&
    cov.refs.concat(cov.guardedRefs).indexOf("Feedback.actionSpec") >= 0,
    JSON.stringify({ aliases: cov.aliases,
      feedbackRefs: cov.refs.concat(cov.guardedRefs).filter(r => r.indexOf("Feedback") === 0) }));
  ...
  /* --- 第五轮 F02-R3：依赖契约**不止一个类型名**，形状也是契约的一部分 ---
   *
   * `Feedback.TEST_FEEDBACK` 从前只声明成 `object`，而真实消费者直接 `list.map(...)`：
   * 注入 `{}` 后应用照样宣告就绪，进到那一屏才抛 `list.map is not a function`
   * （第四次独立复验 run `20260921T052806` 的阻断证据）。所以这里要静态钉住两件事：
   * 该条是 **array 契约**、声明表里出现的类型取值都在闸门认得的白名单里。 */
  ok("Feedback.TEST_FEEDBACK 声明的是**数组契约**（`array`），不是笼统的 object",
    cov.declaredTypeOf["Feedback.TEST_FEEDBACK"] === "array",
    "实际=" + cov.declaredTypeOf["Feedback.TEST_FEEDBACK"]);
  ok("Feedback.UNDO_WINDOW_MS 仍是 number（数组改制没有顺手改坏常量契约）",
    cov.declaredTypeOf["Feedback.UNDO_WINDOW_MS"] === "number",
    "实际=" + cov.declaredTypeOf["Feedback.UNDO_WINDOW_MS"]);
  ok("类型白名单：声明表里出现的类型取值都在闸门认得的契约内（写错类型不会静默失效）",
    cov.declaredTypes.length > 0 &&
    cov.declaredTypes.every(t => prod.CONTRACT_TYPES.indexOf(t) >= 0),
    JSON.stringify({ declaredTypes: cov.declaredTypes, known: prod.CONTRACT_TYPES }));

  const bogusTypeSource = coreSrc.replace('["Feedback.actionSpec", "function",',
    '["Feedback.actionSpec", "functioxx",');
  ok("（前置）非法类型夹具真的改到了那一条声明（否则下面那条是空断言）",
    bogusTypeSource !== coreSrc &&
    bogusTypeSource.indexOf('["Feedback.actionSpec", "functioxx",') > 0);
  const bogusType = prod.runtimeDependencyCoverage(ROOT, { source: bogusTypeSource });
  ok("反向对照：写成白名单外的类型 ⇒ 这条声明不再被认 ⟹ 引用变 undeclared（fail closed）",
    bogusType.declaredTypeOf["Feedback.actionSpec"] === undefined &&
    bogusType.undeclared.some(u => u.path === "Feedback.actionSpec") &&
    bogusType.problems.length >= 1,
    JSON.stringify({ undeclared: bogusType.undeclared, problems: bogusType.problems }));
  // 反向对照的**对照**：合法类型下同一条声明必须被正常识别 ——
  // 否则上面的「变红」可能只是「这张表本来就不认 TEST_FEEDBACK」。
  ok("对照：写成合法类型 `array` 时该声明被正常识别（上面的变红不是「从不识别」）",
    cov.declaredTypeOf["Feedback.actionSpec"] === "function" &&
    cov.declared.indexOf("Feedback.actionSpec") >= 0 &&
    cov.undeclared.length === 0,
    JSON.stringify({ declaredType: cov.declaredTypeOf["Feedback.TEST_FEEDBACK"],
      undeclared: cov.undeclared }));

  const inject = function (snippet) {
    return prod.runtimeDependencyCoverage(ROOT, {
      source: coreSrc.replace("  function uid() {", "  function uid() {\n" + snippet)
    });
  };

  const injected = inject("    const pick = Lib.brandNewThing();");
  ok("反向对照：未声明的 Lib.brandNewThing 直接调用会被报出来（闭合判定有牙齿）",
    injected.undeclared.length === 1 && injected.undeclared[0].path === "brandNewThing",
    JSON.stringify(injected.undeclared));
  // 同一件事的另一面：**带守卫**的引用必须被豁免并如实列进 guardedRefs。
  const guardedInjected = inject("    if (Lib.brandNewThing) return Lib.brandNewThing();");
  ok("反向对照：带守卫的可选引用被豁免，且出现在 guardedRefs 里（可选 ≠ 不需要声明）",
    guardedInjected.undeclared.length === 0 &&
    guardedInjected.guardedRefs.indexOf("brandNewThing") >= 0,
    JSON.stringify({ undeclared: guardedInjected.undeclared, guarded: guardedInjected.guardedRefs }));

  /* --- 模块**别名路径**的反向对照（第三轮新增，F02 的出口判据）---
   * 「`Lib.newThing` 会被报出来」只证明了 `Lib` 这条路径。F02 的缺口在**别名**上：
   * 短别名一旦与别的含义同名，整条路径就退出判定。下面三条分别证明：
   *   ① 唯一别名调用未声明成员 ⇒ 归一后进入 undeclared；
   *   ② 直接命名空间调用未声明成员 ⇒ 进入 undeclared（对照组）；
   *   ③ 同名歧义 ⇒ `problems` 非空，**不许**返回闭合。
   */
  const viaAlias = inject("    const feedbackApi = FeedbackLib;\n    feedbackApi.independentUndeclaredMember();");
  ok("反向对照（别名路径）：`const feedbackApi = FeedbackLib; feedbackApi.<未声明成员>()` " +
    "⇒ 归一成 `Feedback.independentUndeclaredMember` 并进入 undeclared",
    viaAlias.undeclared.length === 1 &&
    viaAlias.undeclared[0].path === "Feedback.independentUndeclaredMember",
    JSON.stringify({ undeclared: viaAlias.undeclared, problems: viaAlias.problems }));

  const viaDirect = inject("    FeedbackLib.independentUndeclaredMember();");
  ok("反向对照（直接路径）：`FeedbackLib.<未声明成员>()` ⇒ 同样进入 undeclared",
    viaDirect.undeclared.length === 1 &&
    viaDirect.undeclared[0].path === "Feedback.independentUndeclaredMember",
    JSON.stringify(viaDirect.undeclared));

  const ambiguousSource = coreSrc
  ...
  ok("（前置）同名歧义夹具真的改到了两处（否则下面那条是空断言）",
    ambiguousSource !== coreSrc &&
    /const auditAlias = FeedbackLib;/.test(ambiguousSource) &&
    /const auditAlias = nativeReminderStatus;/.test(ambiguousSource));
  const ambiguous = prod.runtimeDependencyCoverage(ROOT, { source: ambiguousSource });
  ok("反向对照（同名歧义）：证不了归属 ⇒ `problems` 必须非空，" +
    "而不是「只报告歧义但仍视为闭合」",
    ambiguous.ambiguousAliases.indexOf("auditAlias") >= 0 &&
    ambiguous.problems.some(p => p.indexOf("ambiguous-alias:auditAlias") === 0) &&
    ambiguous.problems.some(p => p.indexOf("ambiguous-ref:auditAlias.") === 0),
    JSON.stringify({ ambiguousAliases: ambiguous.ambiguousAliases,
      ambiguousRefs: ambiguous.ambiguousRefs, problems: ambiguous.problems }));

  /* --- 根对象守卫**不是**成员守卫（第四轮，独立复验 F02-R2 的阻断点）---
   * `Ns ? Ns.m()` / `Ns && Ns.m()` 只证明**模块对象**在，不证明 `m` 在；
   * 而每个命名空间根本身就是必需声明（闸门已保证它存在）⇒ 这种形态下的成员
   * 必须进声明表。扫描器从前把这类形态当「成员守卫」豁免，于是只换成员名就能
   * 绕过矩阵（复验在真实调用上实测 `undeclared=[] problems=[]`）。
   * 下面三条分别证明：别名形态、直接命名空间形态都必须变红；
   * 而**成员级**守卫仍算可选（否则会把可选能力误判成必需）。
   */
  const guardAliasSource = coreSrc.replace("  function uid() {",
    "  function uid() {\n    const feedbackApi = FeedbackLib;\n    const st = feedbackApi ? feedbackApi.independentGuardedMember() : null;");
  ok("（前置）根对象守卫（别名）夹具真的改到了那行真实调用（否则下面那条是空断言）",
    guardAliasSource !== coreSrc &&
    guardAliasSource.indexOf("feedbackApi.independentGuardedMember") >= 0);
  const guardAlias = prod.runtimeDependencyCoverage(ROOT, { source: guardAliasSource });
  ok("反向对照（根守卫·别名）：`feedbackApi ? feedbackApi.<未声明成员>()` ⇒ " +
    "进入 undeclared 且 problems 点名，不进 guardedRefs",
    guardAlias.undeclared.length === 1 &&
    guardAlias.undeclared[0].path === "Feedback.independentGuardedMember" &&
    guardAlias.guardedRefs.indexOf("Feedback.independentGuardedMember") < 0 &&
    guardAlias.problems.some(p =>
      p.indexOf("undeclared-dependency:Feedback.independentGuardedMember") === 0),
    JSON.stringify({ undeclared: guardAlias.undeclared,
      guarded: guardAlias.guardedRefs, problems: guardAlias.problems }));

  const guardDirectSource = coreSrc.replace(
  ...
  ok("（前置）根对象守卫（直接命名空间）夹具真的改到了那行真实调用（否则下面那条是空断言）",
    guardDirectSource !== coreSrc &&
    guardDirectSource.indexOf("FeedbackLib.independentGuardedMember") >= 0);
  const guardDirect = prod.runtimeDependencyCoverage(ROOT, { source: guardDirectSource });
  ok("反向对照（根守卫·直接命名空间）：`FeedbackLib ? FeedbackLib.<未声明成员>() : null` ⇒ " +
    "同样进入 undeclared（该缺口与别名解析无关）",
    guardDirect.undeclared.length === 1 &&
    guardDirect.undeclared[0].path === "Feedback.independentGuardedMember" &&
    guardDirect.problems.length >= 1,
    JSON.stringify({ undeclared: guardDirect.undeclared,
      guarded: guardDirect.guardedRefs, problems: guardDirect.problems }));

  const memberGuardSource = coreSrc.replace("  function uid() {",
    "  function uid() {\n    if (FeedbackLib.independentOptionalMember) return FeedbackLib.independentOptionalMember();");
  const memberGuard = prod.runtimeDependencyCoverage(ROOT, { source: memberGuardSource });
  ok("对照：**成员级**守卫仍算可选（`if (Ns.member)` 进 guardedRefs，不进 undeclared）" +
    "—— 没有这一条，「所有守卫都不豁免」也能让上面两条变绿，那是另一种错",
