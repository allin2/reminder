...
#
# WAL 判据（本版修正）：
#   不用「WAL 字节数增长」当「发生了提交」的代理判据 —— leveldb 在 reuse_logs 恢复路径上
#   会追加一个 count=0 的空 WriteBatch（19 B：7 B 记录头 + 12 B 载荷），字节数每轮稳定 +19
#   但零键操作、序号不前进（2026-09-22 真机实测，曾因此产生一次假红）。
#   真判据是 **WriteBatch 里的操作条数 OPS**：OPS>0 才算「产生了提交」。
#   判定交给 `scripts/verification/idb-wal-delta.py`（遍历**全部**新增记录后汇总），
#   并且**fail closed**：解析不完整 / WAL 轮转 / VERDICT 缺失 ⇒ 一律记失败，
#   不得再当作「信息项」放过。退出码一致性也自检（0=无操作 2=有操作 3=无法判定）。
#
# 变量一律写成 `${VAR}` 花括号形式：
#   本脚本正文含全角标点，在 UTF-8 locale（LC_CTYPE≠C）下 bash 会把紧邻 `$VAR` 的
#   多字节字符当成变量名的一部分 ⇒ `set -u` 下直接 unbound variable 并中断。
#   实测（2026-09-22）：LC_ALL=C.UTF-8 下，裸写 BASE_WAL 紧跟全角左括号时，
#   bash 把它整体当成变量名 BASE_WAL（ 去查 ⇒ unbound variable 并中断执行。
#   本版把全脚本的变量引用统一为 `${VAR}`，并对 [BASE_WAL,A_RSHA,C_WAL,KIND,WAL_REMOTE] 等
#   全部生效，脚本可在任意 locale 下跑完。
#
...
mkdir -p "${WALTMP}"
WALDIR="/data/data/${PKG}/app_webview/Default/IndexedDB/https_localhost_0.indexeddb.leveldb"
WAL_REMOTE="${WALDIR}/000003.log"
walpull() { # $1 = 本地目标
  ${ADB} exec-out run-as "${PKG}" cat "${WAL_REMOTE}" > "$1" 2>/dev/null
  [ -s "$1" ]
}

# WAL 轮转检测：leveldb 写满后会新建编号更高的 `.log` 并改写 CURRENT。
# `.log` 文件名集合一旦变化，就说明「权威 WAL 已不是 000003.log」——
# 此时只盯 000003.log 的增量会把「写进了新日志」漏判成「没有提交」。
# 所以集合变化 ⇒ fail closed。
wal_logset() {
  ${ADB} shell "run-as ${PKG} ls '${WALDIR}'" 2>/dev/null | tr -d '\r' | grep -E '\.log$' | sort | tr '\n' ' '
}
...
# 单轮 WAL 判定：跑 idb-wal-delta.py（遍历全部新增记录），按 VERDICT fail closed。
# 依赖全局：WAL_PREV / WALTMP / RUN / BASE_WAL / C_WAL / FAILED
wal_verdict() { # $1 = 轮次
  local i="$1" now out rc ops kind verdict
  now="${WALTMP}/wal-round-${i}.bin"
  if ! walpull "${now}"; then
    echo "  ✗ WAL 拉取失败（${WAL_REMOTE}）⇒ fail closed"
    FAILED=1
    return
  fi
  if [ -z "${WAL_PREV}" ] || [ ! -f "${WAL_PREV}" ]; then
    echo "  ✗ 无上一份 WAL 可比（上一时点拉取失败）⇒ fail closed，本轮不作判定"
    FAILED=1
    WAL_PREV="${now}"
    return
  fi

  out="${RUN}/wal-delta-round-${i}.txt"
  "${PY}" scripts/verification/idb-wal-delta.py "${WAL_PREV}" "${now}" > "${out}" 2>&1
  rc=$?
  ops="$(grep '^OPS=' "${out}" | cut -d= -f2)"
  kind="$(grep '^RECORD_KIND=' "${out}" | cut -d= -f2)"
  verdict="$(grep '^VERDICT=' "${out}" | cut -d= -f2)"

  echo "  WAL 字节：${BASE_WAL} -> ${C_WAL}（相对 A 时点累计 +$((C_WAL - BASE_WAL)) B；逐轮明细见 wal-delta-round-${i}.txt）"

  case "${verdict}" in
    NO_OPERATIONS)
      echo "  ✓ 新增记录零键操作（OPS=${ops}，RECORD_KIND=${kind}）⇒ 本轮冷启动没有产生任何提交"
      ;;
    HAS_OPERATIONS)
      echo "  ✗ 新增记录含 ${ops} 条真实操作（RECORD_KIND=${kind}）⇒ 冷启动仍写回了数据，幂等未生效"
      FAILED=1
      ;;
    *)
      echo "  ✗ 无法判定（VERDICT=[${verdict}] RECORD_KIND=[${kind}]）⇒ fail closed，按失败计；详见 ${out}"
      FAILED=1
      ;;
  esac

  # 退出码自检：0=无操作 / 2=有操作 / 3=无法判定，必须与 VERDICT 一一对应。
  case "${rc}:${verdict}" in
    0:NO_OPERATIONS|2:HAS_OPERATIONS|3:INCONCLUSIVE) : ;;
    *) echo "  ✗ 判定自相矛盾：退出码 ${rc} 与 VERDICT=[${verdict}] 不符 ⇒ fail closed"; FAILED=1 ;;
  esac
...
BASE_WAL="$(fwals "${A_F}")"
WAL_LOGSET_A="$(wal_logset)"
echo "BASE_WAL=${BASE_WAL}（冷启动前 WAL 字节数；下面每轮与它比，看有没有新增提交）"
echo "WAL_LOGSET_A=[${WAL_LOGSET_A}]（权威 WAL 文件名集合；每轮复查，变了即判轮转⇒fail closed）"
...
for i in $(seq 1 "${ROUNDS}"); do
  ...
  #    真判据是解析新增记录里的 **操作条数 OPS**（由 idb-wal-delta.py 汇总全部新增记录）：
  #    >0 才算「产生了提交」。旧代码的签名是 count=2 的真 put（每轮 +3276 B = 2×1638 B
  #    状态 JSON），用 OPS>0 判它照样报红 ⇒ 判据有牙齿（真机历史里的 count=5/count=4
  #    批量已验证会报红）。
  C_WAL="$(fwals "${C_F}")"
  WAL_LOGSET_NOW="$(wal_logset)"
  if [ "${WAL_LOGSET_NOW}" != "${WAL_LOGSET_A}" ]; then
    echo "  ✗ WAL 文件集合发生变化（A=[${WAL_LOGSET_A}] 现在=[${WAL_LOGSET_NOW}]）⇒ 发生轮转，000003.log 已不是权威 WAL ⇒ fail closed"
    FAILED=1
  fi
  wal_verdict "${i}"
  echo ""
