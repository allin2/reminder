...
// v39：P3-I-R 修复「绑定失败后不可恢复」（事件、内容、表单、诊断、待整理、提示条六处绑定
//   改为事务式：失败整体回滚、成功后才置位、重试不翻倍），并把首页告知条、启动轻摘要与
//   到期谓词迁出为 `lib/app-notices.js`（新增运行时脚本）。三个既有脚本内容变了、
//   又新增一支 ⇒ 必须换缓存名，否则已安装用户继续吃旧内容与旧清单。
...
// v49：回前台读原生状态后，若仍有能力未验证，在 1.5s / 5s 补读（厂商开关异步生效）；设置面板开着时随原生状态变化重绘；更新运行时缓存。
// v48：「允许完全后台运行（防冻结）」改为首页必须提示的一步；首页按 homeDone 判断（点进设置页不算解决，需系统回读后台能力或 60 秒测试确认）；更新运行时缓存。
// v47：到点卡片动作按钮主文字不折行（系统大字体下「我知道了」曾被拆成两行）、副说明均衡折行；快捷时间计算抽为可单测的纯函数；更新运行时缓存。
// v46：UI 改版方案 A —— 首页状态式标题、卡片动作分层、常驻底部输入条、录入快捷时间、未来页分组紧凑行；更新运行时缓存。
// v45：首页设置卡片只为关键缺口出现且可关闭；提醒设置改为清单；「我的」页重新分组、去掉 PRD 入口；新增使用说明；更新运行时缓存。
...
const CACHE = "attention-inbox-v49";
/**
