#!/usr/bin/env python3
"""5-layer resource closure verification for P3-I.
...
APK_PATH = ROOT / "releases/candidates/20260923T1910-p3i-candidate/app-debug.apk"
...
def main():
    ...
    if len(files) != 38 or len(mismatches) != 0:
        print(f"ASSERTION FAILED in 5-layer check: expected 38 files, got {len(files)}, {len(mismatches)} mismatches", file=sys.stderr)
        sys.exit(1)

    print("5-layer resource closure SUCCESS: 38/38 exact matches with genuine intermediate hashes!")
