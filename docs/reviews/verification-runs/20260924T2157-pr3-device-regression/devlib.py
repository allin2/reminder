"""PR3 真机回归共用工具：ADB、WebView CDP、IDB 记录级快照、UI 查找与截图。

只读/写入边界：本模块本身不写业务数据；写入由各步骤脚本显式调用业务命令完成。
证据只保存哈希与隔离事项字段，不保存用户真实事项内容；原始快照与隐私截图另存到 RAW_PRIVATE（仓库外）。
"""
import hashlib
import json
import os
...
RAW_PRIVATE = pathlib.Path(os.environ.get("PR3_PRIVATE_DIR", "/private/tmp/pr3-device-regression-20260924T2157/raw_private"))
...
def ui_nodes():
    sh("uiautomator dump /sdcard/pr3-ui.xml", timeout=60)
    xml = sh("cat /sdcard/pr3-ui.xml")
...
def own_nodes(nodes, tokens=("R3", "A15"), labels=("安心收件箱", "我知道了", "稍后 2 小时", "完成", "稍后", "展开", "收起")):
