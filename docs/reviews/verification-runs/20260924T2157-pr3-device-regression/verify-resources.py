#!/usr/bin/env python3
"""5-layer resource closure verification for PR #3 regression candidate.
...
Also asserts that sw.js inside the candidate APK defines CACHE = "attention-inbox-v43".
"""
import hashlib
import json
...
APK_PATH = ROOT / "releases/candidates/20260924T2157-pr3-regression-candidate/app-debug.apk"
...
def main():
    ...
    sw_version_ok = (cache_name == "attention-inbox-v43")
    ...
    print(f"sw.js cache name: {cache_name}, matches v43: {sw_version_ok}")

    if len(files) != 39 or len(mismatches) != 0 or not sw_version_ok:
        print(f"ASSERTION FAILED in 5-layer check: expected 39 files, got {len(files)}, {len(mismatches)} mismatches, sw_ok={sw_version_ok}", file=sys.stderr)
        sys.exit(1)

    print("5-layer resource closure SUCCESS: 39/39 exact matches with genuine intermediate hashes and v43 sw.js cache!")
