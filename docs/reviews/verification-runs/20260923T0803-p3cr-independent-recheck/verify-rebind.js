/* Independent P3-C-R acceptance A/B. Loads the real production combination without editing product files. */
"use strict";
...
(async () => {
  ...
  assert.strictEqual(rebound.conflictResult, false);
  assert.strictEqual(rebound.before, "waiting");
  assert.strictEqual(rebound.during, "waiting");
  assert.strictEqual(rebound.unrelatedBefore, true);
  assert.strictEqual(rebound.unrelatedAfter, true);
  assert.strictEqual(rebound.actionResult, true);
  console.log(JSON.stringify({ verdict: "P3-C-R rebind invariants passed", control, rebound }, null, 2));
