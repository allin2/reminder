...
| ANR-07 | 隐私与打扰边界 | 关闭弹条持久化 `dismissedUntil` 并抑制 30 分钟内补充提醒；隐私模式隐藏正文；不绕过系统勿扰；**本插件的三个通知渠道不使用全屏 Intent** |
| ANR-08 | 档位与闹钟的边界 | 全屏闹钟**仅归「关键」档所有**；待整理、普通档、重要档一律只用通知渠道。关键档首次提醒走全屏闹钟，其余 7 次补充仍走通知。契约见 [关键档全屏闹钟](./android-fullscreen-alarm.md) |
...
> **2026-09-16 修订**：本规格早前写「不使用全屏通知」。那句话是对**本插件这三个渠道**的准确描述，
> 今天依然成立——渠道本身不设全屏 Intent。
>
> 但仓库后来新增了一条**独立通道**：关键档的全屏闹钟（`SystemBridge` +
> `AlarmActivity`），它不经过本插件的渠道，也不改变上面这条约束。两者边界见 ANR-08 与
> [关键档全屏闹钟](./android-fullscreen-alarm.md)。此前的表述容易被读成
> 「产品永不用全屏」，那是错的——分档规则是：**只有关键档用，且只用首次**。
