...
(function () {
  ...
  /* ---------- 共用展示能力：唯一来源 lib/app-ui.js ---------- */

  /**
   * DOM 查询 / 弹层开关 / toast / 确认框 / 外链白名单。
   *
   * `createUi` 只**装配**：不注册监听、不开定时器、不碰业务状态，所以在这里
   * （IIFE 顶部）实例化不产生副作用。取不到 `AppUi` 就退回 `{}`，让下面几个名字成为
   * `undefined` —— 这与会话里其它必需模块（`Lib.nextRepeatTrigger` 等）的约定一致：
   * **别名不做兜底**，缺件由 `assertRuntimeDependencies()` 在业务启动之前拦下并报出
   * 「哪一支脚本没起来、后果是什么」，而不是在这里悄悄换一套实现继续跑。
   *
   * `isFeedbackSuppressed` 必须按**取值函数**注入：装配发生在 IIFE 顶部，那一刻
   * `suppressUserFeedback` 必然是 `false`；按值传进来会把整段重放期的抑制逻辑
   * 钉死成「永不抑制」，等于把 M1 的重复提示问题放回来。
   */
  const AppUi = Lib.AppUi || {};
  const appUi = typeof AppUi.createUi === "function"
   ...
   *
   * 刻意**不包含** `AttentionNativeReminders` / `Capacitor`：原生桥是「旁路能力」，
   * 纯 Web 环境里它本来就不存在，那是**正常状态**而不是装配错误。
   *
  ...
  const REQUIRED_RUNTIME_EXPORTS = [
    ...
    ["Feedback", "object",
      "lib/feedback.js 未加载 —— 动作语义与撤销窗口缺失，撤销会退化成兜底时长"],
    ["DeliveryEvidence", "object",
      "lib/delivery-evidence.js 未加载 —— 送达证据无法合并，「提醒结果」这一行会消失"]
  ];
   ...
   *
   * 纯函数：只读 `AttentionLib`，不碰 DOM、不写状态、不排钟。因此可以直接在测试里
   * 用不同的 `Lib` 形状反复调用，也可以在启动前安全地跑一次。
   */
  function assertRuntimeDependencies() {
    ...
    for (let i = 0; i < REQUIRED_RUNTIME_EXPORTS.length; i++) {
      ...
      const got = readPath(Lib, path0);
      if (typeof got !== want) {
        problems.push({
          path: path0,
          expected: want,
          actual: got === null ? "null" : typeof got,
          why: why
        });
      }
    }
   ...
   */
  const DatePrimitives = (Lib && Lib.DatePrimitives) || {};
   ...
   */
  const UiFormat = (Lib && Lib.UiFormat) || {};
  ...
  // 兼容别名：同一来源的委托，不另存算法体
  const startOfDay = DatePrimitives.startOfDay;
  const endOfDay = DatePrimitives.endOfDay;
  const addDays = DatePrimitives.addDays;
  const sameDay = DatePrimitives.sameDay;
  function fmtTime(ts) {
  ...
  const nextWeekend = DatePrimitives.nextWeekend;
  ...
  // lib/date-utils.js（命名空间）
  const applyClock = DatePrimitives.applyClock;
  const lastDayOfMonth = DatePrimitives.lastDayOfMonth;
  const nthWeekdayInMonth = DatePrimitives.nthWeekdayInMonth;
  const weekdayOfNextWeek = DatePrimitives.weekdayOfNextWeek;
  const dayOfMonthIn = DatePrimitives.dayOfMonthIn;
  const nextDayOfMonth = DatePrimitives.nextDayOfMonth;
   ...
   * 现在直接取模块导出：取不到就是装配错误，由 `assertRuntimeDependencies()` 拦下。
   */
  const nextRepeatTrigger = Lib.nextRepeatTrigger;

  // lib/repeat.js
  const repeatLabel = Lib.repeatLabel;
  const nextRepeatPreview = Lib.nextRepeatPreview;
   ...
   * 现在只保留唯一来源；取不到 = 装配错误，由 assertRuntimeDependencies() 在启动前拦下。
   */
  const parseChineseTime = Lib.parseChineseTime;
  const cnInt = Lib.cnInt;
  const hasSpecificTimeWord = Lib.hasSpecificTimeWord;
  ...
  function startApp() {
    ...
    // 「半套规则」跑起来，而且不报错。
    const depProblems = assertRuntimeDependencies();
  ...
  if (typeof globalThis !== "undefined") {
    globalThis.__ATTENTION_INBOX__ = {
      ...
      nthWeekdayOfNextMonthByInstant: DatePrimitives.nthWeekdayOfNextMonthByInstant,
      nthWeekdayOfNextMonthByDay: DatePrimitives.nthWeekdayOfNextMonthByDay,
      nthWeekdayInMonth,
      lastDayOfMonth,
      repeatLabel,
      findSimilarItems,
