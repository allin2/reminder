...
(function (root, factory) {
  ...
  function evidenceStatusFor(item, ctx) {
    ...
    const planned = keys.map(k => ({ key: k, at: keyAt(k), state: events[k] && events[k].state }));
    // **只认当前轮的证据**：`triggerAt` 之前的键属于被改掉时间的旧承诺。
    // 它们即便是 delivered 也不能代表「这一次」已收到 —— 这正是 R4 被误报的那一步。
    const base = Number(item.triggerAt) || 0;
    const inRound = planned.filter(p => !base || (p.at != null && p.at >= base));
