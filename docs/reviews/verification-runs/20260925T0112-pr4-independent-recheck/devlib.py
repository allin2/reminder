...
RAW_PRIVATE = pathlib.Path(os.environ.get("A15_PRIVATE_DIR", "/private/tmp/claude-501/-Users-qlyf-Developer-reminder/5f5c9ea0-11fa-4ed0-9d48-1e8bad8e2724/scratchpad/pr4-recheck-private"))
...
def create_text_item(text, priority="normal"):
    """只填 #capText，由应用解析时间（不手工选时间），返回新事项关键字段。"""
    js = """
(async () => {
  const A = __ATTENTION_INBOX__;
  const before = new Set(A.state.items.map(i => i.id));
  A.openCapture(); await new Promise(r => setTimeout(r, 300));
  const t = document.querySelector('#capText'); t.value = %s; t.dispatchEvent(new Event('input', {bubbles: true}));
  document.querySelector('#capPriority .chip[data-p="' + %s + '"]').click();
  A.saveItemFromForm();
  for (let i = 0; i < 50 && A.inflightDepth() > 0; i++) await new Promise(r => setTimeout(r, 100));
  await new Promise(r => setTimeout(r, 800));
  const it = A.state.items.find(i => !before.has(i.id));
  return it ? {id: it.id, title: it.title, status: it.status, rev: it.rev, triggerAt: it.triggerAt, priority: it.priority, review_status: it.review_status} : null;
})()""" % (json.dumps(text), json.dumps(priority))
    return cdp_eval(js)


def delete_item(iid):
    return cdp_eval("(async()=>{const A=__ATTENTION_INBOX__;const ok=A.deleteItem(%s);A.hideAlert&&A.hideAlert();for(let i=0;i<50&&A.inflightDepth()>0;i++)await new Promise(r=>setTimeout(r,100));await A.saveAsync();return ok})()" % json.dumps(iid))


def pending_for(iid):
    return cdp_eval("(async()=>{const r=await Capacitor.Plugins.LocalNotifications.getPending();return r.notifications.filter(n=>n.extra&&n.extra.itemId===%s).map(n=>({id:n.id,at:n.schedule&&n.schedule.at,event:n.extra.event}))})()" % json.dumps(iid))


def last_status():
    return cdp_eval("(()=>{const s=__ATTENTION_INBOX__.getNativeReminderStatus();return {removed:s.removedDeliveredIds,cleanupError:s.deliveredCleanupError,errors:s.errors}})()")
