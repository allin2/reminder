...
def main():
    ...
    try:
        ...
        (async () => {
            ...
            app.renderHome();
            const card = document.querySelector('.card[data-id="it-delivered"] .card-title');
            if (card) card.click();
            await new Promise(r => setTimeout(r, 50));
            const clickResult = {
                cardFound: !!card,
                sheetOpen: document.querySelector('#sheetDetail').classList.contains('open'),
                detailId: app.state.ui.detailId,
                rowVisible: document.querySelector('#detailBody').innerHTML.includes('系统已接收这次提醒（只代表系统收到了这次提醒）')
            };
            return { ready: ready, results: results, clickResult };
        })()
        """
        ...
        click = data.get("clickResult", {})
        checks.append(data.get("ready") is True)
        checks.append(click.get("cardFound") is True and click.get("sheetOpen") is True and click.get("detailId") == "it-delivered" and click.get("rowVisible") is True)
        if all(checks):
