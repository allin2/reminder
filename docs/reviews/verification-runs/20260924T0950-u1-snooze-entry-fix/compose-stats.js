...
/** 本轮（U1 修复 run）报告公布的数字，用于对账（不是断言目标，只用来报告差异）。
 * 相对 20260923T2336-p3ir-delivery 的变化：openSnoozeSheet 由 E（11 行实现体）
 * 变为 C（3 行薄转发，其中 1 行为 return 委托），E 37→36 / 233→220（naive），
 * C 175→176；eventsRuntimeDeps 以 isItemActionPending / rejectPendingItemCommand
 * 替换 openSnoozeSheet 注入，A 1389→1390。D 的 +2 行差异为 naive 边界假象，
 * 已在上一轮 run 披露，本轮报告口径如实沿用。 */
const PUBLISHED = {
  total: { funcs: 254, lines: 2370 },
  A: { funcs: 22, lines: 1390 },
  B: { funcs: 9, lines: 295 },
  C: { funcs: 176, lines: 432 },
  D: { funcs: 11, lines: 33 },
  E: { funcs: 36, lines: 220 },
};
