...
ROOT = pathlib.Path(__file__).resolve().parents[5]  # 本脚本比验收方原件深一层（snooze-entry/）
...
with tempfile.TemporaryDirectory(prefix="u1-snooze-fixed-") as profile:
    ...
    try:
        ...
        result = client.evaluate("""(async () => {
          ...
          // R4：重选 chip 后确认恰好一次 → triggerAt 推进到今晚 20:00，面板关闭
        ...
        checks = [
            ...
            result["customAfterReopenEmpty"] is True and result["chipOnAfterReopen"] is False and result["stillOpen"] is True,
