...
def run_matrix_trial(config_label, trial_num):
    ...
    while get_dev_time_ms() < wait_deadline:
        ...
        act_lines = [l.strip() for l in acts_curr.splitlines() if "topResumed" in l or "AlarmActivity" in l]
        ...
        # 检查是否投递
        if alarm_activity_time and not delivered_time:
            delivered_time = alarm_activity_time
...
def extract_logcat_evidence(logcat_path):
    ...
    with open(logcat_path, 'r', encoding='utf-8', errors='ignore') as f:
        for idx, line in enumerate(f, 1):
            if ('AttentionAlarm deliver' in line or 'AlarmRingService: deliver' in line) and not deliver_line:
