...
(function (root, factory) {
  ...
  function createStorage(opts) {
    ...
    return {
      ...
      /**
       * P2-C-R：**重新尝试建立权威后端**（恢复面板的「重试恢复」按钮用）。
       *
       * `ensure()` 一旦选定 backend 就**不再回头**（`if (backend === "idb" || backend === "local") return;`），
       * 于是「`openDb()` 失败 ⇒ 整体降到 local」之后，用户点重试也只会再读一次 localStorage ——
       * 权威后端**永远不会被重新尝试**。清掉这个决定，让下一次 `load()` 重新 `openDb()`。
       *
       * 刻意不动 `mem.value` / `lsKey`：重试只是「再试一次打开」，不改变任何已存字节。
       * 返回 false 表示这个实例被显式钉在 memory（测试），没有可重试的权威后端。
       */
      reopen() {
