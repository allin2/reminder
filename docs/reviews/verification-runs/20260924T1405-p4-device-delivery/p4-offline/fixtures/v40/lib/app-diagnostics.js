...
(function (root, factory) {
  ...
  function createAppDiagnostics(deps) {
    ...
    /** 绑定的**事务体**：全部注册动作都在这里，成功后才由 `bind()` 置位。 */
    function bindControls() {
      ...
      if (req) listen(req, "click", labRequestNotify);
      const openNotify = query("#labOpenNotify");
      if (openNotify) listen(openNotify, "click", () => openSystemSetting("notify"));
      const openExact = query("#labOpenExact");
      if (openExact) listen(openExact, "click", () => openSystemSetting("exact"));
      const openBattery = query("#labOpenBattery");
      if (openBattery) listen(openBattery, "click", () => openSystemSetting("battery"));
      const openOverlay = query("#labOpenOverlay");
      if (openOverlay) listen(openOverlay, "click", () => openSystemSetting("overlay"));
      const openFsi = query("#labOpenFsi");
      if (openFsi) listen(openFsi, "click", () => openSystemSetting("fsi"));
      const openAutoStart = query("#labOpenAutoStart");
      if (openAutoStart) listen(openAutoStart, "click", () => openBackgroundGuide("permissions"));
      const openBackground = query("#labOpenBackground");
      if (openBackground) listen(openBackground, "click", () => openBackgroundGuide("background"));
      const traceTest = query("#labTraceTest");
      if (traceTest) listen(traceTest, "click", labTraceTest);
      const testNow = query("#labTestNow");
      if (testNow) listen(testNow, "click", labShowNow);
      const test10 = query("#labTest10s");
      if (test10) listen(test10, "click", () => labScheduleAlarm(10000, "10 秒"));
      const test60 = query("#labTest60s");
      if (test60) listen(test60, "click", () => labScheduleAlarm(60000, "1 分钟"));
      const cancel = query("#labCancelAlarm");
      if (cancel) listen(cancel, "click", labCancelAlarms);
      const refresh = query("#labRefresh");
      if (refresh) listen(refresh, "click", refreshNotifyLab);
      const resync = query("#labResync");
      if (resync) {
        listen(resync, "click", async () => {
          labLog("正在重新对账…");
          try {
