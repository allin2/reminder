...
(function () {
  ...
  // L01 / L05：review 的手选时间仍归 core；capture 表单对应状态由 appCapture 管理。
  ...
  function applyParsedState(parsed) {
    ...
    // `Object.assign` below may replace `settings.ai` with a partial legacy value.
    // Retain the model-owned defaults first so the subsequent nested merge never
    // accidentally merges that partial value with itself.
    ...
    if (!state.settings.ai || typeof state.settings.ai !== "object") {
      state.settings.ai = modelAiDefaults;
    } else {
      state.settings.ai = Object.assign(modelAiDefaults, state.settings.ai);
    }
  ...
  function ensureReviewSettings() {
    const reviewDefaults = appModel.createInitialState().settings.review;
    if (!state.settings.review) {
      state.settings.review = reviewDefaults;
    } else {
      // 升级旧参数：45min×1 → 60min×2
      if (state.settings.review.followupMs == null || state.settings.review.followupMs === 45 * 60 * 1000) {
        state.settings.review.followupMs = reviewDefaults.followupMs;
      }
      if (state.settings.review.maxFollowups == null || state.settings.review.maxFollowups === 1) {
        state.settings.review.maxFollowups = reviewDefaults.maxFollowups;
      }
  ...
  function applyNewItem(id, fields) {
    ...
    if (!id || state.items.some(x => x.id === id)) return false;
  ...
  function maybeDailySummary() {
    ...
    const important = state.items.filter(it => it.createdAt >= today &&
      (it.priority === "important" || it.priority === "critical")).length;
    state.settings.lastSummaryAt = Date.now();
    save();
    if (added >= 3 || important > 0) {
      const msg = "今天新增 " + added + " 条未来关注" + (important ? "，其中 " + important + " 条重要" : "");
      toast(msg);
      showSystemNotification({ title: "安心收件箱 · 今日摘要", body: msg,
  ...
  function seed() {
    ...
    const defaultSettings = appModel.createInitialState().settings;
  ...
  if (typeof globalThis !== "undefined") {
    globalThis.__ATTENTION_INBOX__ = {
      ...
      markTriggerPicked: (v) => appCapture.markTriggerPicked(v),
      ...
      resetItemSheet,
      closeAllSheets,
      get formSession() { return appCapture ? appCapture.session() : null; },
      clearAlert: () => { alertItem = null; },
      // 依赖闸门失败时不暴露可写业务 state；仅给诊断 hook 一个不可变空视图，保持旧测试的可观测形状。
      ...
      get captureSurface() {
        if (!appCapture) return { missing: true };
        return { hasBind: typeof appCapture.bind === "function", hasSession: typeof appCapture.session === "function",
          hasSnapshot: typeof appCapture.snapshotItemForm === "function", hasOpen: typeof appCapture.openCapture === "function",
          hasEdit: typeof appCapture.openEditItem === "function", hasHint: typeof appCapture.updateParseHint === "function" };
      },
      get setupSurface() {
