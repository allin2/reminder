...
async function run() {
  ...
  section("B2. P1 装配闸门：唯一来源矩阵闭合，缺模块时明确失败且不进入业务启动");
  {
    ...
    ok("P3-A 模型实例 API 双向闭合（源码里的 `appModel.<x>` 与契约逐项对齐）",
      ...
      modelCov.used.length === 9 && modelCov.declared.length === 9, JSON.stringify(modelCov));
    const modelContractRemoved = baseSource.replace('"resolveDeliveryMode", "bumpRev", "isTerminal", "hasKnownRev"]',
      '"resolveDeliveryMode", "bumpRev", "isTerminal"]');
    ...
    // Independent P3-B acceptance: every namespace/factory/instance failure must fail closed.
    const persistenceContract = ["currentPayload", "writeSnapshot", "runCommit", "saveAsync", "save", "loadAsync", "loadSync", "load",
      "replayPendingSnapshot", "authoritativeWritesAllowed", "authoritySnapshot", "reopen", "committedItemById", "getStorage"];
    const persistenceSource = fs.readFileSync(path.join(ROOT, "lib/app-persistence.js"), "utf8");
    const persistenceTemp = (name, suffix) => {
      const file = path.join(os.tmpdir(), "attention-independent-p3b-" + name + ".js");
      fs.writeFileSync(file, persistenceSource + "\n" + suffix + "\n", "utf8"); return file;
    };
    const assertPersistenceBootFailure = async (name, sourceFile, expectedPath) => {
      const bad = sourceFile === null
        ? await bootCombination({ scripts: indexScripts.filter(s => s !== "lib/app-persistence.js") })
        : await bootCombination({ overrides: { "lib/app-persistence.js": sourceFile } });
      const problems = bad.app && bad.app.startupFailure ? bad.app.startupFailure() : [];
      ok("独立 P3-B " + name + "：ready=false 且点名 " + expectedPath,
        bad.ready === false && problems.some(p => p.path === expectedPath) &&
        problems.every(p => p.path.indexOf("AppPersistence") === 0), JSON.stringify(problems));
      ok("独立 P3-B " + name + "：fail closed 前零 IDB put / 排程 / 业务心跳",
        bad.disk.puts === 0 && bad.env.calls.scheduleAlarm.length === 0 &&
        bad.intervals.indexOf(15000) < 0 && bad.intervals.indexOf(2000) < 0,
        JSON.stringify({ puts: bad.disk.puts, scheduleAlarm: bad.env.calls.scheduleAlarm.length, intervals: bad.intervals }));
    };
    await assertPersistenceBootFailure("缺 lib/app-persistence.js", null, "AppPersistence");
    await assertPersistenceBootFailure("空 AppPersistence 命名空间", persistenceTemp("empty-namespace",
      "AttentionLib.AppPersistence = {};"), "AppPersistence.createAppPersistence");
    await assertPersistenceBootFailure("工厂抛错", persistenceTemp("factory-throw",
      "AttentionLib.AppPersistence = { createAppPersistence: function(){ throw new Error('persistence-boom'); } };"), "AppPersistence.createAppPersistence()");
    for (let persistenceIndex = 0; persistenceIndex < persistenceContract.length; persistenceIndex++) {
      const missing = persistenceContract[persistenceIndex];
      const members = persistenceContract.filter(name => name !== missing).map(name => JSON.stringify(name) + ": function(){}").join(",");
      await assertPersistenceBootFailure("缺实例成员 " + missing, persistenceTemp("missing-" + missing,
        "AttentionLib.AppPersistence = { createAppPersistence: function(){ return {" + members + "}; } };"), "AppPersistence." + missing);
    }
