...
try:
    ...
    out = c.e("""(async () => {
      try {
        ...
        const memoryStatus = a.state.items.find(x => x.id === 'review-save-fault-probe')?.review_status;
        ...
        const persistedStatus = persisted?.items?.find(x => x.id === 'review-save-fault-probe')?.review_status;
        return {
            memoryStatus, persistedStatus, reviewIndexAfterFault, completedAfterFault,
            ready: true,
            hasContract,
            entryRendered,
            sheetOpen,
    ...
    time.sleep(1.0)
    after = c.e("""(async()=>{const a=window.__ATTENTION_INBOX__;await a.ready();return {status:a.state.items.find(x=>x.id==='review-save-fault-probe')?.review_status || null,ready:true};})()""")
    print(json.dumps({"beforeReload": out, "afterReload": after}, ensure_ascii=False))
    if not out or not after or after.get("status") != "NEEDS_REVIEW" or out.get("memoryStatus") != "REVIEWED" or out.get("persistedStatus") != "NEEDS_REVIEW" or not out.get("completedAfterFault"):
        raise SystemExit(1)
finally:
    p.terminate()
    srv.shutdown()
