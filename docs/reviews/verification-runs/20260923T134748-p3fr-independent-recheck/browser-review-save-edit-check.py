...
try:
    ...
    out = c.e("""(async () => {
      try {
        ...
        const faultTitle = q('#reviewTitle'); if (faultTitle) faultTitle.value = '编辑后标题';
        const faultConfirm = q('#reviewSave');
        ...
        const retryConfirm = q('#reviewSave');
    ...
    after = c.e("""(async()=>{
      ...
      return {
        ready: true,
        reloadedStatus: it ? it.review_status : null, reloadedTitle: it ? it.title : null
      };
    })()""")
    ...
    expected_checks = (
        ...
        after.get("reloadedStatus") == "REVIEWED" and after.get("reloadedTitle") == "编辑后标题"
    )
