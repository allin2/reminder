...
- **机制攻克**：vivo OriginOS 具备极其严格的 `fast_freezer` cgroup 冻结策略。若在排程后立刻调用 `input keyevent KEYCODE_POWER`，前端异步防抖定时器会被休眠挂起。在脚本中改为主动 `await A.syncNativeRemindersNow({ forceRebuild: true })`，确保系统底层立即完成 `setAlarmClock` 写入。
...
3. **vivo 全流程**（`20260925T035800Z`）共执行 3 轮：
   - 第 1 轮（12:09）A1 / A2 / A4 / A5 / D2 / F1 / F4 共 7 项 FAIL（设备上原有 2 条到点事项影响计数，脚本随后改为按基线差值判断）；
   - 第 2 轮（12:26）F1 FAIL：锁屏后全屏闹钟没有弹出；
   - 第 3 轮（12:32）全部通过，前提是脚本在锁屏前主动调用 `syncNativeRemindersNow({ forceRebuild: true })`。
4. **遗留风险（非本次改版引入，建议另开任务）**：vivo 上「刚创建关键事项就立刻锁屏」时，原生排程可能因前端同步防抖加 OriginOS 冻结机制而来不及写入。这是真实用户路径，F1 第 3 轮是靠测试脚本强制同步才通过的，**不能证明产品在该路径上可靠**。
